...
class ElectraEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([ElectraLayer(config) for _ in range(config.num_hidden_layers-1)])
...
class ElectraModel(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings: GBST = GBST(
            ...
            downsample_factor=config.downsample_factor,  # the final downsample factor by which the sequence length will decrease by
        ...
        self.upsampling = nn.Upsample(scale_factor=config.downsample_factor, mode='nearest')
    ...
    def forward(
        ...
        hidden_states, attention_mask, unscaled_hidden_states = self.embeddings(
        ...
        extended_unscaled_attention_mask = self.get_extended_attention_mask(unscaled_attention_mask, input_shape, device)
        ...
        if self.upsample_output:
            ...
            # hidden_states = self.encoder_last_layer(hidden_states, extended_attention_mask,
            #                                         None, None, None, None, False)
...
class ElectraForPreTraining(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        discriminator_sequence_output = discriminator_hidden_states

        logits: torch.FloatTensor = self.discriminator_predictions(discriminator_sequence_output)
